    ...
    <div class="lg:px-8 lg:py-16 pb-20 pt-5">
      ...
      <div class="flex justify-between items-center mx-1 mt-2 text-sm">
        <div class="mb-8 flex items-center space-x-2">
          ...
          <i class="fa-solid fa-angle-right text-primary"></i>
          <p @click="$inertia.get(route('products.show', product.id))" class="cursor-pointer text-primary">
            {{ product.name }}</p>
        </div>
        <Link v-if="$page.props.auth.user.is_admin" :href="route('products.edit', product.id)">
        <PrimaryButton><i @click="$inertia.get(route('products.edit', product.id))"
            class="fa-solid fa-pen text-sm text-white mr-2"></i>Editar</PrimaryButton>
        </Link>
      </div>
      <div class="rounded-lg lg:mx-24 mt-4">
        <div class="lg:grid grid-cols-2">
          ...
          <div class="bg-[#D9D9D9] rounded-md mx-36 relative hidden lg:block">
            <figure class="">
              <img :src="images[0][0]?.original_url" alt="Product image"
                class="lg:w-[550px] lg:h-96 h-64 object-contain bg-no-repeat" @mouseover="showZoom"
                @mousemove="updateZoomPosition" @mouseout="hideZoom">
            </figure>
            <div class="border border-gray-600 h-36 w-36 -bottom-0 right-0 rounded-lg bg-cover absolute" ref="zoomBox"
              v-if="isZoomVisible" :style="zoomBoxStyle"></div>

            <figure v-if="images[1][0]?.original_url"
              class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute  -bottom-24 left-0 lg:top-0 lg:-left-40 cursor-pointer rounded-lg border hover:border-[#9a9a9a]">
              <img :src="images[1][0]?.original_url" alt="image" class="w-full h-full bg-cover bg-no-repeat rounded-lg">
            </figure>

            <figure v-if="images[2][0]?.original_url"
              class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-32 lg:top-32 lg:-left-40 cursor-pointer rounded-lg border hover:border-[#9a9a9a]">
              <img :src="images[2][0]?.original_url" alt="image" class="w-full h-full bg-cover bg-no-repeat rounded-lg">
            </figure>

            <figure v-if="images[3][0]?.original_url"
              class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-64 lg:top-64 lg:-left-40 cursor-pointer rounded-lg border hover:border-[#9a9a9a]">
              <img :src="images[3][0]?.original_url" alt="image" class="w-full h-full bg-cover bg-no-repeat rounded-lg">
            </figure>
          ...
          <div class="py-3 px-6">
            ...
            <div class="lg:hidden mb-40 mt-3 relative">
              <figure class="bg-[#D9D9D9] rounded-lg">
                <img :src="images[0][0]?.original_url" class="object-contain bg-no-repeat w-96 h-56" alt="Product image">
              </figure>
              <figure
                class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute  -bottom-24 left-0 lg:top-0 lg:-left-40 cursor-pointer rounded-lg border hover:border-[#9a9a9a]">
                <img :src="images[1][0]?.original_url" alt="image" class="w-full h-full object-contain bg-no-repeat">
              </figure>

              <figure
                class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-28 lg:top-32 lg:-left-40 cursor-pointer rounded-lg border hover:border-[#9a9a9a]">
                <img :src="images[2][0]?.original_url" alt="image" class="w-full h-full object-contain bg-no-repeat">
              </figure>

              <figure
                class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-56 lg:top-64 lg:-left-40 cursor-pointer rounded-lg border hover:border-[#9a9a9a]">
                <img :src="images[3][0]?.original_url" alt="image" class="w-full h-full object-contain bg-no-repeat">
              </figure>
            </div>
            ...
            <div class="relative inline-block">
              <h3 class="text-primary font-bold text-xl my-3">${{ product?.is_percentage ? ((product?.price * (100 -
                product?.discount) * 0.01)).toString().split('.')[0]
            ...
            : ((product?.price - product?.discount) * form.quantity).toString().split('.')[0]
              ...
              <h3 class="text-primary font-bold text-xs absolute top-3 -right-4">{{ ((
                product.price).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")).toString().split('.')[1] ?? '00' }}</h3>
            </div>
            ...
            <div class="flex flex-col mt-3">
              ...
              <div class="flex space-x-2">
                <input v-model="form.color" value="black" type="radio" name="radio_group"
                  class="bg-black border border-gray-300 rounded-full checked:bg-black checked:border-[#D90537]" />
                <input v-model="form.color" value="white" type="radio" name="radio_group"
                  class="bg-white border border-gray-300 rounded-full checked:bg-white checked:border-[#D90537]" />
                <input v-model="form.color" value="red" type="radio" name="radio_group"
                  class="bg-red-600 border border-gray-300 rounded-full checked:bg-red-600 checked:border-[#D90537]" />
                <input v-model="form.color" value="blue" type="radio" name="radio_group"
                  class="bg-blue-600 border border-gray-300 rounded-full checked:bg-blue-600 checked:border-[#D90537]" />
              </div>
            </div>

            <div class="flex items-center space-x-4 my-4">
              <p class="font-bold text-sm">Cantidad:</p>
              <input v-model="form.quantity" type="number" min="1" :max="product.stock"
            ...
            <p class="text-secondary text-xs mt-3">*El pedido debe solicitarse con al menos una semana de anticipación
            </p>
          </div>
        </div>
        ...
        <div class="mt-12 lg:mx-48 lg:grid grid-cols-2 mx-3">
          <div>
            ...
            <ul class="ml-3 flex flex-col">
              <li class="flex space-x-1 text-gray-600" v-for="feature in product.features?.toString().split(',')"
