        ...
        <div class="lg:flex space-x-5">
          <div class="bg-[#D9D9D9] rounded-md lg:ml-36 relative lg:w-[550px] lg:h-96 mx-1">
            <figure class="lg:w-[550px] lg:h-96 h-64">
              <img ref="previewImage" src="@/../../public/images/add-image.png" alt="Product image"
                class="lg:w-[550px] lg:h-96 h-64 object-contain bg-no-repeat opacity-60">
            </figure>

            <p class="text-white text-xs rounded-full px-3 py-1 absolute top-2 right-2 bg-black z-20">Portada del producto
            </p>

            <i class="fa-solid fa-camera text-lg text-primary font-bold rounded-full absolute bottom-3 right-16 bg-[#CCCCCC] py-2 px-3 cursor-pointer"
              @click="openFileInput('fileInput')"></i>
            <input @input="form.media = $event.target.files[0]" type="file" ref="fileInput" style="display: none"
              @change="handleFileChange" />

            <i
              class="fa-solid fa-trash-can text-lg text-primary font-bold rounded-full absolute bottom-3 right-4 bg-[#CCCCCC] py-2 px-3 cursor-pointer"></i>

            <div @click="openFileInput('fileInput1')"
              class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-0 lg:top-0 lg:-left-40 cursor-pointer rounded-md border hover:border-[#9a9a9a]">
              <img ref="thumbnail1" src="@/../../public/images/add-image.png" alt="image"
                class="w-full h-full bg-cover bg-no-repeat opacity-60 rounded-lg">
              <input @input="form.media1 = $event.target.files[0]" type="file" ref="fileInput1" style="display: none"
                @change="handleFileChange1" />
            </div>

            <div @click="openFileInput('fileInput2')"
              class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-32 lg:top-32 lg:-left-40 cursor-pointer rounded-md border hover:border-[#9a9a9a]">
              <img ref="thumbnail2" src="@/../../public/images/add-image.png" alt="image"
                class="w-full h-full bg-cover bg-no-repeat opacity-60 rounded-lg">
              <input @input="form.media2 = $event.target.files[0]" type="file" ref="fileInput2" style="display: none"
                @change="handleFileChange2" />
            </div>

            <div @click="openFileInput('fileInput3')"
              class="lg:w-36 lg:h-28 w-28 h-20 bg-[#D9D9D9] absolute -bottom-24 left-64 lg:top-64 lg:-left-40 cursor-pointer rounded-md border hover:border-[#9a9a9a]">
              <img ref="thumbnail3" src="@/../../public/images/add-image.png" alt="image"
                class="w-full h-full bg-cover bg-no-repeat opacity-60 rounded-lg">
              <input @input="form.media3 = $event.target.files[0]" type="file" ref="fileInput3" style="display: none"
          ...
          <div class="lg:w-1/2 space-y-3 pr-4 mt-28 lg:mt-0">
            ...
            <div>
              <select v-model="form.category" class="input lg:w-1/3 h-11" name="" id="">
                <option v-for="(category, index) in categories" :key="category" :value="category" :disabled="index == 0">
                  {{ category }}</option>
              </select>
            ...
            <div>
              <select v-model="form.material" class="input lg:w-1/3 h-11" name="" id="">
                <option v-for="(material, index) in materials" :key="material" :value="material" :disabled="index == 0">{{
                  material }}</option>
              </select>
            ...
            <div>
              <div v-if="form.has_discount" class="flex items-center space-x-2">
                ...
                <input class="text-sm checked:bg-primary text-gray-600 bg-transparent mr-1" v-model="form.is_percentage"
                  type="radio" name="es_porcentage" :value="true"> %
                <input class="text-sm checked:bg-primary text-gray-600 bg-transparent mr-1" v-model="form.is_percentage"
                  type="radio" name="es_porcentage" :value="false"> $
