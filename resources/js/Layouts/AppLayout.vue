        ...
        <div class="min-h-screen bg-transparent">
            ...
            <nav class="bg-black border-b border-gray-200 lg:hidden py-2 px-5">
                ...
                <div>
                    <div class="relative z-50">
                        <input type="text" placeholder="Buscar" class="input bg-gray-100 pr-6 h-8 mt-2" v-model="searchTerm"
                            @input="search" @focus="isFocused = true" @blur="delayBlur" ref="searchInput" />
                        <i class="fa-solid fa-magnifying-glass absolute text-sm text-[#9A9A9A] top-[9px] right-2"></i>
                    </div>

                    <!-- Lista de resultados -->
                    <ul v-if="searchResults.length && isFocused" style="z-index: 9999;"
                        class="bg-gray-200 border border-gray-300 shadow-md rounded-md absolute w-[350px] p-1">
                        <li style="z-index: 9999;" class="cursor-pointer hover:bg-gray-300 px-3 rounded-full py-1"
                            v-for="result in searchResults" :key="result.id" @click="selectResult(result)">
                ...
                <div class="w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex justify-between h-20 mt-3 lg:mt-0">
                        <div class="flex space-x-7 items-center">
                            ...
                            <div class="relative">
                                <ThirthButton @click="showCategories = !showCategories"
                                    class="h-7 lg:flex items-center hidden lg:block">Categorias <i
                                        class="fa-solid fa-angle-down ml-2"></i></ThirthButton>
                                <ul style="z-index: 9999;" v-if="showCategories"
                                    class="bg-gray-200 border border-gray-300 shadow-md rounded-md absolute p-1 w-40 text-sm ">
                                    <li style="z-index: 9999;"
                                        class="cursor-pointer hover:bg-gray-300 px-3 py-1 rounded-full"
                                        v-for="category in categories" :key="category" @click="selectResult(result)">
                            ...
                            <div>
                                <div class="relative z-50">
                                    <input type="text" placeholder="Buscar" class="input lg:w-[600px] pr-6"
                                        v-model="searchTerm" @input="search" @focus="isFocused = true" @blur="delayBlur"
                                        ref="searchInput" />
                                    <i
                                        class="fa-solid fa-magnifying-glass absolute text-sm text-[#9A9A9A] top-[9px] right-2"></i>
                                </div>

                                <!-- Lista de resultados -->
                                <ul style="z-index: 9999;" v-if="searchResults.length && isFocused"
                                    class="bg-gray-200 border border-gray-300 shadow-md rounded-md absolute w-[600px] p-1">
                                    <li style="z-index: 9999;"
                                        class="cursor-pointer hover:bg-gray-300 px-3 rounded-full py-1"
                            ...
                            <div class="hidden lg:block">
                                <p v-if="$page.props.auth?.user?.name" class="text-primary mx-4">Bienvenido {{
                                    $page.props.auth?.user?.name }}</p>
                                <Link v-else :href="login">
                                <p class="text-primary mx-4 hover:underline">Iniciar sesion</p>
                                </Link>
                            </div>
                            ...
                            <div
                                class="rounded-full bg-primary h-8 w-8 flex items-center justify-center cursor-pointer relative">
                                <i class="fa-solid fa-cart-shopping text-sm text-white"></i>
                                <div v-if="$page.props.auth.user.cart_products.length > 0"
                                    class="absolute rounded-full border-2 border-white -top-1 -right-2 bg-secondary text-white text-[9px] h-4 w-4 flex justify-center text-center">
                                    {{ $page.props.auth.user.cart_products.length }}</div>
                            </div>
                            </Link>
            ...
            <main class="bg-gray-100">
                <div class="overflow-hidden bg-gray-100 flex justify-between">
                    <aside v-if="$page.props.auth.user.is_admin" class="hidden lg:block"
                        :class="showSideNav ? 'w-[10%] relative  transition-transform ease-out duration-500' : ''">
                        <SideNav v-if="showSideNav" />
                        <!-- --- Pestaña con sidenav oculto --- -->
                        <div v-if="!showSideNav" @click="showSideNavToggle"
                            class="bg-[#c9c9c9] hover:scale-105 text-gray-600 w-7 h-12 cursor-pointer flex items-center justify-center rounded-r-lg">
                            <i class="fa-solid fa-angles-right"></i>
                        </div>
                        <!-- --- Pestaña con sinav acivo --- -->
                        <div v-if="showSideNav" @click="showSideNavToggle"
                            class="absolute -top-7 -right-6 bg-[#c9c9c9] hover:scale-105 text-gray-600 w-7 h-12 cursor-pointer mt-7 flex items-center justify-center rounded-r-lg">
                ...
                <footer class="hidden lg:block">
                    <div class="bg-[#1a1a1a] h-auto w-full lg:grid grid-cols-3 text-center text-white relative">
                        ...
                        <a href="https://dtw.com.mx/" target="_blank">
                            <figure class="my-5 absolute -bottom-5 right-4 flex items-center ">
                                <img class="w-[20%]" src="@/../../public/images/logo-dtw.png" alt="Logo" />
                                <span class="ml-3">Digital TW</span>
                            </figure>
                        </a>
                    </div>
                    <p class="bg-[#1a1a1a] text-white px-3 pb-2">Copyright 2023. Emblems3dUSA todos los derechos reservados.
                        Políticas de privacidad y administración de cookies</p>
                ...
                <nav class="lg:hidden fixed bottom-0 w-full z-10">
                    <div class="w-full h-16 flex justify-center px-4 bg-black space-x-6">
                        ...
                        :class="route().current('dashboard') ? 'text-primary border-t-[4px] border-[#D90537]' : 'text-white'"
                        class="flex flex-col justify-center text-center px-2">
                        <i class="fa-solid fa-house text-xl"></i>
                        <p class="text-xs">Inicio</p>
                    </div>
                    <div @click="$inertia.get(route('profile.show'))"
                        :class="route().current('profile.*') ? 'text-primary border-t-[4px] border-[#D90537]' : 'text-white'"
                        class="flex flex-col justify-center text-center px-2">
                        <i class="fa-regular fa-circle-user text-xl"></i>
                        <p class="text-xs">Mi cuenta</p>
                    </div>
                    <div @click="$inertia.get(route('carts.index'))"
                        :class="route().current('carts.*') ? 'text-primary border-t-[4px] border-[#D90537]' : 'text-white'"
                        class="flex flex-col justify-center text-center px-2">
                        <i class="fa-solid fa-cart-shopping text-xl"></i>
                        <p class="text-xs">Carrito</p>
                    </div>

                    <div @click="$inertia.get(route('products.index'))"
                        :class="route().current('products.*') ? 'text-primary border-t-[4px] border-[#D90537]' : 'text-white'"
                        class="flex flex-col justify-center text-center px-2">
